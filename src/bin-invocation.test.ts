...
before(() => {
  try {
    ...
    if (!binPath) {
      throw new Error(
        `bin 'verify' not present after install. Contents: ${readdirSync(binDir).join(", ")}`,
      );
    }
  } catch (e) {
    ...
    console.error(`[bin-invocation] setup failed: ${(e as Error).message}`);
  }
});
...
test("invoking through a symlinked path still runs — no silent no-op", () => {
  ...
  try {
    ...
    try {
      ...
      assert.fail(
        `could not create a symlink to exercise the bin path: ${(e as Error).message}. ` +
    ...
    const r = spawnSync(
      process.execPath,
      [join(link, "cli.js"), "00000000-0000-0000-0000-000000000000"],
    ...
    assert.notEqual(
      combined.trim(),
      "",
      "invoked through a symlink the CLI produced NO output — main() never ran, " +
        "which is the fail-open bug this test exists to catch",
    );
    assert.notEqual(
      r.status,
      0,
      "a nonsense id returned SUCCESS through a symlinked path — failing open",
    );
  } finally {
...
test("bin fails closed on a nonsense id — never exit 0", async () => {
  ...
  assert.notEqual(
    r.code,
    0,
    "a nonsense receipt id returned SUCCESS through the bin — the verifier is failing open",
  );
