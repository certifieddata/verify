...
export interface CertV2Payload {
  ...
  manifest?: { engine?: string; record_count?: number; [k: string]: unknown };
  [k: string]: unknown;
}

/** The signature as production actually serves it, or as a bare base64 string. */
export type CertV2Signature =
  | string
  | { value?: string; sig?: string; signature?: string; alg?: string; key_id?: string };
...
export function isCertV2(doc: unknown): boolean {
  if (!doc || typeof doc !== "object") return false;
  const d = doc as Record<string, unknown>;
  if (d.schema_version === "cert.v2") return true;
 ...
 */
function unwrapSignature(raw: unknown): { b64: string; alg?: string; keyId?: string } | null {
 ...
 */
export function toEnvelope(doc: Record<string, unknown>): CertV2Envelope | string {
  const hasNested = !!doc.payload && typeof doc.payload === "object";
  const payload = (hasNested ? doc.payload : doc) as CertV2Payload;
  ...
  return {
    ...
    signature_alg:
      unwrapped.alg ?? (typeof doc.signature_alg === "string" ? doc.signature_alg : undefined),
    envelope_key_id:
      unwrapped.keyId ?? (typeof doc.signing_key_id === "string" ? doc.signing_key_id : undefined),
  };
}
...
export async function verifyCertificateV2(
  ...
  if (typeof env === "string") return finish(result, "MALFORMED", env);
  ...
  if (env.envelope_key_id && env.envelope_key_id !== keyId) {
    return finish(
      result,
      "MALFORMED",
      `envelope names key_id ${env.envelope_key_id} but the signed payload names ${keyId}`,
    );
  }
  ...
  if (!sigOk) {
    return finish(
      result,
      "INVALID",
      "ed25519 signature does not verify against canonicalized cert.v2 payload",
    );
  }
  ...
  if (datasetPath) {
    ...
    if (actualHex !== payload.artifact_hash.toLowerCase()) {
      ...
      return finish(
        result,
        "DATASET_MISMATCH",
        `artifact hash mismatch (expected sha256:${payload.artifact_hash.toLowerCase()}, got ${result.dataset_hash_actual})`,
      );
    }
 ...
 */
function isEd25519(algorithm: string | undefined): boolean {
  return typeof algorithm === "string" && algorithm.toLowerCase() === "ed25519";
}

function validateV2Shape(p: CertV2Payload): string | null {
  if (!p || typeof p !== "object") return "cert.v2 payload is not an object";
  for (const f of ["certificate_id", "issued_at", "artifact_hash", "issuer"] as const) {
    if (p[f] === undefined || p[f] === null) return `missing required field: ${f}`;
  }
  if (!p.issuer || typeof p.issuer !== "object") return "issuer must be an object";
...
function blankV2Result(): VerifyResult {
  return {
    verdict: "MALFORMED",
