...
export type Resolution =
  | { kind: ArtifactKind; via: string }
  | { kind: "ambiguous"; via: string }
  | { kind: "not_found"; via: string }
  | { kind: "transport_error"; via: string };

export async function resolveArtifactKind(
  target: string,
  opts: { offline?: boolean; certApiBase?: string; receiptApiBase?: string } = {},
): Promise<Resolution> {
  // 1. Prefix hints — cert-family prefixes are certificates.
  if (/^(cert_|scert_)/.test(target)) return { kind: "certificate", via: "id-prefix" };
  ...
  if (looksLocal && target !== "-") {
    try {
      ...
      if (parsed.receipt && parsed.storedReceiptHash) return { kind: "receipt", via: "local-envelope" };
      return { kind: "certificate", via: "local-default" };
    } catch {
      return { kind: "certificate", via: "local-unreadable-default" };
    }
  }
  if (target === "-") return { kind: "certificate", via: "stdin-default" };

  // 2. Bare id online — probe both endpoints. Offline cannot probe.
  if (opts.offline) return { kind: "certificate", via: "offline-default" };
  ...
  const probe = async (url: string): Promise<"exists" | "missing" | "error"> => {
    try {
  ...
  if (cert === "error" || rcpt === "error") {
    ...
    if (cert === "exists" && rcpt !== "exists") return { kind: "certificate", via: "probe" };
    if (rcpt === "exists" && cert !== "exists") return { kind: "receipt", via: "probe" };
    return { kind: "transport_error", via: "probe" };
  }
  if (cert === "exists" && rcpt === "exists") return { kind: "ambiguous", via: "probe" };
