...
export async function verifyCertificate(cert, trustedKeys, datasetPath) {
    ...
    if (!sigBytes)
        return finish(result, "MALFORMED", "signature is not valid base64");
    ...
    if (!sigOk)
        return finish(result, "INVALID", "ed25519 signature does not verify against canonicalized payload");
    if (datasetPath) {
...
function validateShape(c) {
    ...
    if (c.schema_version !== "cert.v1")
        return `unsupported schema_version: ${c.schema_version}`;
    if (!/^sha256:[0-9a-f]{64}$/i.test(c.dataset_hash))
        return "dataset_hash must be sha256:<64-hex>";
    if (typeof c.rows !== "number" || typeof c.columns !== "number")
...
function blankResult(cert) {
    return {
        verdict: "MALFORMED",
        certification_id: cert.certification_id ?? null,
        key_id: cert.key_id ?? null,
        issuer: cert.issuer ?? null,
        algorithm: cert.algorithm ?? null,
        signed_at: cert.timestamp ?? null,
        dataset_hash_expected: cert.dataset_hash ?? null,
