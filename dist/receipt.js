...
export async function fetchReceipt(idOrPathOrUrl, opts = {}) {
    if (idOrPathOrUrl === "-")
        return parseEnvelope(await readStdin());
    if (idOrPathOrUrl.endsWith(".json") ||
...
function parseEnvelope(body) {
    ...
    if (parsed.receipt && typeof parsed.receipt === "object") {
        return {
            ...
            storedHash: typeof parsed.storedReceiptHash === "string" ? parsed.storedReceiptHash : null,
            serverReported: {
                valid: typeof parsed.valid === "boolean" ? parsed.valid : undefined,
                signatureValid: typeof parsed.signatureValid === "boolean" ? parsed.signatureValid : undefined,
 ...
 */
export async function loadReceiptKey(opts = {}) {
    if (opts.keyFile)
        return readFile(opts.keyFile, "utf8");
    if (opts.offline) {
    ...
    const res = await fetch(url);
    if (!res.ok) {
        throw new Error(`public key unavailable (HTTP ${res.status} from ${url}) — cannot verify independently`);
    }
...
export function verifyReceiptEnvelope(env, publicKeyPem) {
    const p = env.payload;
    const result = {
    ...
    try {
        ...
        result.checks.key_trust = "fail";
        result.verdict = "UNKNOWN_KEY";
        result.reason = `cannot parse published public key: ${e.message}`;
        return result;
    }
