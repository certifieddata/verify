...
function assertNoAbort(r, label) {
    assert.ok(!/(Assertion failed|UV_HANDLE_CLOSING)/.test(r.stderr + r.stdout), `${label}: process aborted instead of exiting — ${r.stderr.trim()}`);
