...
function parseKeyDoc(body) {
    ...
    const keys = parsed.keys.map((raw) => {
        ...
        const material = (raw.public_key ?? raw.public_key_pem ?? raw.public_key_raw_b64url);
        ...
        return {
            ...
            algorithm: String(raw.algorithm ?? "").toLowerCase(),
            revoked_at: raw.revoked_at ?? revokedAtFor.get(keyId) ?? null,
        };
    });
    return { ...parsed, issuer: String(parsed.issuer ?? ""), keys };
}
/**
 ...
 */
function buildRevocationIndex(doc) {
    const out = new Map();
    for (const field of ["revoked", "retired"]) {
        ...
        for (const entry of list) {
            ...
            throw new Error(`invalid key document: unreadable entry in ${field}[] — refusing to ignore a revocation record`);
        }
    }
    return out;
}
